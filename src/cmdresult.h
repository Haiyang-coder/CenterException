...
typedef struct{
	int iResult; //查詢結果
    int iLineSize;//总行数
	int iLineCurrent;//现在存储的行数
	int iClomuSize;//每一行的列数
	int iCurrentPage;//现在的页数
	char CloName[PAGE_COLU_MAX][D_NSP_PDCSMCS_TICS_BUF_LEN]; //每一列的列名
    PersonnalDataLine psDataMap[PAGE_ROWS_MAX][PAGE_COLU_MAX];//每一行的结构体数组指针
}PersonnalDataStruct;

...
CThreadPool::CThreadPool()
{
    ...
    minThreaad = std::stoi(SystemIni.GetValue("threadpool", "MIN_THREAD", "5"));
    maxThreaad = std::stoi(SystemIni.GetValue("threadpool", "MAX_THREAD", "10"));

    // 实例化任务队列
    m_taskQ = new CThreadQueue;
    do
    {
        // 初始化线程池
        m_minNum = minThreaad;
        m_maxNum = maxThreaad;
        m_busyNum = 0;
        m_aliveNum = minThreaad;

        // 根据线程的最大上限给线程数组分配内存
        m_threadIDs = new pthread_t[maxThreaad];
        ...
        // 初始化
        std::memset(m_threadIDs, 0, sizeof(pthread_t) * maxThreaad);
        // 初始化互斥锁,条件变量
...
void CThreadPool::addTask(TaskNode task)
{
    if (m_shutdown)
    {
        return;
...
int CThreadPool::getAliveNumber()
{
    int threadNum = 0;
...
void *CThreadPool::worker(void *arg)
{
    ...
    while (true)
    {
        ...
        // 从任务队列中取出一个任务
        TaskNode task = pool->m_taskQ->takeTask();
        // 工作的线程+1
        pool->m_busyNum++;
        // 线程池解锁
        pthread_mutex_unlock(&pool->m_lock);

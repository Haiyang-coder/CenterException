...
public:
    ...
    int static SavaDataAsCsv(const CDbData &dataAll, std::string &fullName);
    // 获取数据库的表信息
    const St_NSP_PDCSMCS_TableMap *GetTableMap();
    // 获取模式名
    int GetModelName(std::string &modelName);
...
private:
    ...
    // 获取带提交时间限制的总行数
    unsigned int GetCommitTimeLines(const char *Start, const char *End, const char *tableName, long &RowNumber);
    ...
    int CreateTable(int);
    // 设置数据库的归档，开启作业
    int SetBackUpDB(DMStmt *stDmStmt);
    // 设置完全备份
    int SetCompliBackUp(DMStmt *stDmStmt);
    // 修改完全备份
    int AlterCompliBackUp(DMStmt *stDmStmt);
    // 修改增量备份
    int AlterAddBackUp(DMStmt *stDmStmt);
    // 设置增量备份
    int SetAddBackUp(DMStmt *stDmStmt);
    // 更新完全备份路径
    int UpdateCompliBackUp();
    // 更新增量备份路径
    int UpdateAddBackUp();
    // 初始化查询的数据类，主要是填上表名，类型名，字段名
    ...
    // 获取查询语句的结果
    int static GetDataByExcuteSql(DMStmt *stDmStmt, CDbData &data);

private:
    CSimpleIniEx m_simpleIni;
